  	...
  	<body>
		...
		<div class="container">
			...
			<div class="row">
				...
				mysqli_stmt_prepare($quarterResults, "SELECT * FROM `photoQuarters` where `yearId` = ? and `quarter` = ?");
				mysqli_stmt_bind_param($quarterResults, "ii", $year['id'], $_GET['quarter']);
				...
				<div class="span9 rel">
		    		...
		    		<div class="span9">
		    			<div class="well">
		    				...
		    				<?php if($cuser['edit'] == 1 || $editor == true){ ?><a href="photos.php?yearStart=<?=$yearStart?>&quarter=<?=$quarter["quarter"]?>" class="btn btn-default pull-right absTR">Add Photo</a><?php } ?>
		    			</div>
		    		</div>
		    		<div class="span9">
					<div class="well">
				...
				$photoResults = mysqli_stmt_init($mysqli);
					mysqli_stmt_prepare($photoResults, "SELECT * FROM `photoAll` where `yearId` = ? and `quarter` = ?");
					mysqli_stmt_bind_param($photoResults, "ii", $year['id'], $_GET['quarter']);
					...
					?>
						<img src="<?=$photo["photoUrl"]?>" width="210px" heigth="140px">
				<?php }?>
				</div>
				</div>
