         ...
         <div class="well">
         	...
         	if(isset($_SESSION['username'])){
					$mysqli = new mysqli("localhost", "chscyberweb", $pwwd, "chs");
			 		...
			 		$suggs = $mysqli->query("SELECT * FROM `game_suggestion` ORDER BY `id` DESC");
		      ?>
					<table class="table styled-table scoreboard">
						...
						<tbody>
							...
							while($sug = $suggs->fetch_array()) {
							   $usern = $mysqli->query("SELECT * FROM `game_users` WHERE `id`=".$sug['user']."");
							   $user = $usern->fetch_array();
							   $usern = $user['username'];
							   ?>
							   <tr>
							      <td><?=$sug['id']?></td>
							      <td><a href="player.php?id=<?=$sug['user']?>" title="<?=$user['fname']?> <?=$user['lname']?>"><?=$usern?></a></td>
							      <td><?=$sug['text']?></td>
							   </tr>
							   <?php
							}
