<?php $thisPage="parents"; ?>



<!DOCTYPE html>

<html lang="en">

	<head>

		<title>Cedar High School</title>

    	<?php include("assets/inc/headtag.php") ?>
    	<style type="text/css">
    	.style9 {color: #990000}
        </style>
	</head>

  	<body>

		<?php include("assets/inc/navbar.php") ?>

		

    	<!-- Let the container begin! -->

		<div class="container">

			<div class="row">

			  <?php include("assets/inc/navlist.php") ?>

			  <div class="span9">		

	       			<div class="well">

          				<h3>Parents</h3>

           			</div>

								

								<div id="myCarousel" class="carousel slide" style="width: 100%; margin: 15px auto;">

			            		<div class="carousel-inner">

			              			<div class="item active">

			                			<img src="/assets/img/home-1.jpg" alt="">

			              			</div>

			              			<div class="item">

			                			<img src="/assets/img/home-2.jpg" alt="">

			              			</div>

			              			<div class="item">

			                			<img src="/assets/img/home-3.jpg" alt="">

			              			</div>

													<div class="item">

			                			<img src="/assets/img/home-4.jpg" alt="">

			              			</div>

			            		</div>

			            		<a class="left carousel-control" href="#myCarousel" data-slide="prev">&lsaquo;</a>

			            		<a class="right carousel-control" href="#myCarousel" data-slide="next">&rsaquo;</a>

			       			</div><!--well-->

       		  </div><!--span9-->

				

	   			<div class="span9">

	   				<div class="row-fluid">

	   					

	   					<!-- Perdy slide show -->

	   					<div class="span8">	
 <div class="well"> <h3><span class="style9">Join PFCC</span></h3>
  <p> Hi, parents and teachers of our great students at CHS! We are the PFCC (Parent Faculty Community Club). We are here to assist the faulty and staff with whatever they need to make the students' experience at CHS better. It is $6 to join PFCC. You are welcome to pay that directly to Mrs. Stubbs at the office or online through our website. You are not obligated to be a volunteer, but you are welcome to join us in various school activities if your schedule allows. Also, we are a bonus! Our funds are used entirely to benefit our kids and the clubs and organizations that they join. Dig in your pockets and see if you have $6 per parent or teacher to help make this year a great one! <p>Thanks so much, <p>Sidney Nakken (PFCC President)<p>To pay your fee online <a href="https://secureinstantpayments.com/sip/cart/event.php?EID=999"target="_blank">click here</a></p></p></div><div class="well">

			<h3>Online Fee Payment with Instant Payment</h3>
		          				<p><a href="https://secureinstantpayments.com/sip/cart/event.php?EID=999">Pay Student Fees</a><br>
		          				<a href="http://old.irondistrict.org/registration.html">Registration Documents</a></p>
   				          </div><!--well--><!--well-->
                          
                          <div class="well">
                          
                          <h3><span class="style9">Redmen Pride Nominations</span><br></h3>
                          <p>Submit Redmen Pride nominations online using the link below. Only nominate seniors.<br></p>
                          <a href="https://docs.google.com/a/ironmail.org/forms/d/1IukIgxxmqY7-qt74baE53hPgpBTJ6J8kOeox-FCVb0I/edit"target="_blank">Redmen Pride Nomination Form</a><br>
                          </div><!--well-->
                          
			       			<div class="well">

		          				<h3><a href="http://www.actstudent.org/regist/dates.html">ACT dates</a></h3>
   				  <table width="491" style="text-align:left">
       				              <thead>
       				                <tr>
       				                  <th width="130">Test Date</th>
       				                  <th width="141">Registration Deadline</th>
                                      <th width="204">Late Fee Required</th>
   				                    </tr>
   				                </thead>
       				              <tbody>
       				                                                    <tr>
                                      <td>Sept 8, 2018</td>
                                      <td>Aug 12, 2018</td>
                                      <td>Aug 11-26, 2018</td>
                                    </tr>
                                      <tr>
                                      <td>Oct 27, 2018</td>
                                      <td>Sep 28, 2018</td>
                                      <td>Sep 29-Oct 14, 2018</td>
                                      </tr><tr>
                                      <td>Dec 8, 2018</td>
                                      <td>Nov 2, 2018</td>
                                      <td>Nov 3-19, 2018</td>
                                      </tr><tr>
                                      <td>Feb 9, 2019</td>
                                      <td>Jan 11, 2019</td>
                                      <td>Jan 12-18, 2019</td>
                                      </tr>
                                      <tr>
                                      	<td>Feb 26, 2019 (Jrs. Only)</td>
                                      </tr><tr>
                                      <td>Apr 13, 2019</td>
                                      <td>Mar 8, 2019</td>
                                      <td>Mar 9-25, 2019</td>
                                      </tr>
                                      <tr>
                                        <td>Jun 8, 2019</td>
                                        <td>Mar 3, 2019</td>
                                        <td>May 4-20, 2019</td>
                                      </tr>
                                      <tr>
                                      <td>Jul 13, 2019</td>
                                      <td>Jun 14, 2019</td>
                                      <td>Jun 15-24, 2019</td>
                                      </tr>
   				                </tbody>
   				              </table><br>
                              
                              ACT Code 450-045<br>Click <a href="http://www.actstudent.org/regist/dates.html">here</a> for ACT website
                     
                              
                          </div><!--well-->

	           			     
                      <div class="well">
                     
                       <h3><span class="style9">
                       Sage Testing Results</span><br></h3>
                       <p>Students were given a copy of their results from the SAGE test they took at the end of school last year.   Please follow the link below to access the information on how to read the test results.<br>
                         <a href="general/sageTesting/Parent Guide.pdf">Parent Guide</a><br>
                         <a href="general/sageTesting/Parent Guide Spanish.pdf">Parent Guide Spanish</a><br>
                         <a href="general/sageTesting/Parent Letter.pdf">Parent Letter</a><br>
                       </p>
                      </div>
                      
                      
                      <div class="well">
<h3><span class="style9">A Letter to Graduates and Parents</span><br></h3>

<p>To view the letter to graduates from Principal Dodds, go to <a href="general/201605161014.pdf">this link.</a><br></p>
</div>
                          
                          
                           <div class="well">

		          				<h3>Helpful Links</h3>
		          				<p><a href="http://www.jostens.com/?gclid=CPCQj9XNrMoCFYqGfgodV20PEg"target="_blank">Jostens</a> (for class rings, caps and gowns, letterman jackets)<br>
		          			<a href="suu.edu/concurrent">Concurrent Enrollment</a>
	          				 <br><a href="general/tonys educ 2000 flyer.pdf">Interested in Becoming a Teacher?</a></p>
		          			
                                
   				           </div> <!--well-->

                          <div class="well">
   				            
			                <h3>Dates to Remember</h3>
   				            <ul>
   				            <li>PSAT -   Wednesday, October 10,   2018</li>
   				            <li>End of Qtr. 1 - October 12, 2018</li>
   				            <li>Sterling Scholar applications due - November 2, 2018</li>
   				            <li>College Application Week - November 12-16, 2018</li>
   				            <li>Priority Deadline SUU and most colleges - December 1, 2018</li>
   				            <li>Financial Aid Night: FAFSA Completion Night - December 3, 2018</li>
   				            <li>ASVAB Test (Jr. and Sr.) - December 4, 2018</li>
   				            <li>FINAL deadline for BYU applications - December 15, 2018</li>
   				            <li>ICSD Career Fair (Jrs. only)- December 17, 2018</li>
   				            <li>End of Qtr 2 - December 20, 2018</li>
   				            <li>Winter Break - December 21, 2018-January 1, 2019</li>
                            <li>MLK day-No School - January 21, 2019</li>
                            <li>Deadline for Applications to many Utah 4 year Institutions - February 1, 2019</li>
                            <li>Spring CCR Week-Early Out @ 1:30-No Late Start - February 4-8, 2019</li>
                            <li> *Diploma of Merit Applicatons due - February 15, 2019 </li>
                       			<li>Presidents's Day-No School - February 18, 2019</li>
                            <li>ACT for Juniors- February 26, 2019 (ACT Code 450-045)</li>
                            <li>Final Deadline for Scholarships for SUU - March 1, 2019</li>
                            <li>End of Quarter 3 - March 8, 2019</li>
   				            <li>*Senior Awards Night - Monday, May 20, 2019 @ 6:00 p.m. (auditorium)
   				            <li>Graduation 5:00 PM  @ SUU Centrum - Wednesday, May 22, 2019</li>
   				            </ul>
                          
			              </div><!--well-->
                           
   				           <div class="well">
                           
	          				 <h3>Senior Information</h3>
	          				 <h4>Graduation Information</h4>
		          				<ul>
		          				  <li>Graduation will be held in the SUU Centrum at 5:00 p.m. Wednesday, May 22nd.</li>
          				      </ul>
       				         <h4>Sterling Scholar/ Diploma of Merit</h4>
          				        <ul>
                            <li>Diploma of Merit applications are due in February of 2019.</li>
       				          </ul>
                              
   				          </div><!--well-->
                          
                          <div class="well">

		          				<h3>Scholarship Information<br>
		          				</h3>
		          				<ul>
		          				  <li><a href="http://www.collegexpress.com/">College Express</a><br>
	          				      </li>
		          				  <li><a href="http://www.fastaid.com/">Fast Aid</a><br>
	          				      </li>
		          				  <li><a href="http://www.fastweb.com/">Fast Web</a></li>
		          				  <li><a href="http://www.finaid.org/">Fin Aid</a></li>
		          				  <li><a href="http://www.petersons.com/">Petersons</a></li>
		          				  <li><a href="http://www.scholarships.com/">Scholarships.com</a></li>
		          				  <li><a href="http://www.successacademy.org/SUUScholarships.html">Success Acadamy Scholar</a></li>
		          				  <li><a href="https://www.utahfutures.org/Portal.aspx">Utah Futures</a></li>
		          				  <li><a href="http://www.washk12.org/secondary-education-academics/scholarships">Washington County Scholarship List</a>   				          </li>
	          				</ul>
                            
   				          </div><!--well--><!--well-->
                          
                          <div class="well">

		          				<h3>Mrs. Stubbs Window Hours</h3>
		          				<p>Tuesday<strong>-</strong>Friday 7:30 am to 8:00 am<br>
		          				  Monday-Friday 11:00 am to 12:00 pm<br>
	          				Monday-Friday 2:40 pm to 3:00 pm</p>
          				  </div>
                            <div class="well">

		          				<h3>Notice!</h3>
		          				<p>Visitors, for the saftey of our students only the main doors are unlocked during school hours. All other side doors will remain locked. Please enter through the main doors only and check into the office before you enter the school. Thank you.</p>
                                
   				          </div><!--well-->

	           		  </div><!--span8-->

	   					<!--parent navlist-->

   						<?php include("assets/inc/stuparnav.php") ?>

						<div class="span4">
					  	  <div class="well">

							<h5>Phone Number</h5>

							<ul class="nav nav-list">

									<li>(435) 586-2820</li>
							</ul>
                            
                         	</div><!--/well-->
                   		</div><!--/span4-->
                          	<!--/student navlist-->
                        </div>
   						<!--/row-fluid-->

	       			

	   			</div><!--/span9-->

	</div><!--/row -->



      		<?php include("assets/inc/footer.php") ?>



</div> <!-- /container -->



    	<!-- Yummy JavaScrips

    	================================================== -->

    	<!-- Placed at the end of the document so the pages load faster -->

    	<script src="assets/js/jquery.js"></script>

    	<script src="assets/js/bootstrap.js"></script>

    	<script src="assets/js/bootstrap-carousel.js"></script>



  	</body>

</html>


